...
function Rules({ label }: ITabProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    if (cards && cards[0]) {
      ...
      if (
        cards[0].cardStatus === "Card-active" &&
        cards[4].cardStatus !== "Card-active"
      ) {
  ...
  return (
    <div className="Rules" data-id={label}>
      ...
      <p className="Rules__text">
        SET is an exercise in perceptiveness. <br />
        <br />
        What you need to do is find a SET of three cards out of the twelve cards
        you see:
      </p>
      <TabWrapper>{Utils.renderCards(cards)}</TabWrapper>
      <section className="Rules__section">
        <h2 className="Rules__subheader">Why are those cards a set?</h2>
        <p className="Rules__text">
          Well, each card has four features.
          <br />
          And each feature comes in three variants.
          <br />
          Click on the buttons below to explore all the 81 (3 x 3 x 3 x 3)
          possible cards.
        </p>
        <Card id="green-squiggle-filled-1" cardStatus="Card-inactive" />
        ...
        <p className="Rules__text">
          A SET consists of three cards in which every feature is either the
          same (e.g. all cards are green) or different (e.g.: a green card + a
          red card + a violet card).
        </p>
      </section>
      ...
      <div className="sampleCards">
        ...
        <Card id="green-squiggle-filled-1" cardStatus="Card-inactive" />
        <Card id="red-squiggle-filled-1" cardStatus="Card-inactive" />
        <Card id="violet-squiggle-filled-1" cardStatus="Card-inactive" />
      ...
      <p className="Rules__text">
        Hint: If two cards share a feature and the third one doesn’t, it’s not a
        set.
      </p>
      <h2 className="Rules__subheader">
        Ok, but what's the point of the game?
      </h2>
      <p className="Rules__text">
        {" "}
        Depends. If you play against the computer, the player with more SETs
        wins.
      </p>
      <p className="Rules__text">
        {" "}
        If you play in the solo mode (the only mode currently available here...
        but <em>just you wait!</em>), try to finish the game as fast as you can.
      </p>
    </div>
  );
}
